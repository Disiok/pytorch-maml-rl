...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu', delay=20):
        ...
        self._intrinsic_rewards_list = [[] for _ in range(batch_size)]
        self._mask_list = []
    ...
    def intrinsic_rewards(self):
        if self._intrinsic_rewards is None:
            intrinsic_rewards = np.zeros((len(self), self.batch_size), dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._intrinsic_rewards_list[i])
                intrinsic_rewards[:length, i] = np.stack(np.squeeze(self._intrinsic_rewards_list[i]), axis=0)

            self._intrinsic_rewards = torch.from_numpy(intrinsic_rewards).to(self.device)
    ...
    def append(self, observations, actions, rewards, batch_ids, intrinsic_rewards=None):
        if intrinsic_rewards is not None:
            for observation, action, reward, batch_id, intrinsic_reward in zip(
                    observations, actions, rewards, batch_ids, intrinsic_rewards):
                if batch_id is None:
                    continue
                ...
                self._intrinsic_rewards_list[batch_id].append(intrinsic_reward.astype(np.float32))
        else:
