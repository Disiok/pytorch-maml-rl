...
class MAESNMetaLearner(object):
    ...
    def __init__(self,
        ...
        self.tau = tau
        self.kl_regularization_factor = kl_regularization_factor
        self.to(device)
    ...
    def inner_loss(self, episodes, params=None):
        ...
        latent_advantages = advantages[0]  # Get the first transition [N]

        latent_distribution = self.policy.latent_distribution(episodes.task_id, params=params)
        zs = (latent_distribution.loc + latent_distribution.scale * episodes.noise).detach()

        latent_log_probs = latent_distribution.log_prob(zs)
        latent_log_probs = torch.sum(latent_log_probs, dim=1)
        latent_loss = -torch.mean(latent_log_probs * latent_advantages)
    ...
    def adapt(self, episodes, first_order=False):
        ...
        loss, latent_loss = self.inner_loss(episodes)
    ...
    def surrogate_loss(self, episodes, old_pis=None):
        ...
        for (train_episodes, valid_episodes), old_pi in zip(episodes, old_pis):
            ...
            with torch.set_grad_enabled(old_pi is None):
                ...
                losses.append(loss + self.kl_regularization_factor * kl_regularization)
